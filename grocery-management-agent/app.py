...
def run_review(item_name, description, state):
    ...
    try:
        result = agent.review_item(item_name, description, memory_notes=state["memory_notes"])
...
with gr.Blocks(title="Grocery Management Agent") as demo:
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=5, min_width=380):
            ...
            description = gr.Textbox(
                label="Staff Report",
                placeholder=(
                    "Describe the item's situation — stock on hand, expiry or best-before date, "
                    "storage, how fast it sells, and anything you noticed.\n\n"
                    "e.g. '6 cartons left in aisle 3, use-by is tomorrow, we usually sell about 10 a day.'"
                ),
                lines=6,
            )
    ...
    run_btn.click(
        run_review,
        inputs=[item_name, description, state],
        outputs=[
...
if __name__ == "__main__":
    # On Render, PORT is injected automatically — use it.
    # Locally, omit server_port so Gradio auto-picks a free port.
    launch_kwargs = dict(
        theme=_theme,
        css=CSS,
        js=FORCE_DARK_JS,
    ...
    if render_port:
        launch_kwargs["server_port"] = int(render_port)
